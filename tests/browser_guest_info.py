...
with sync_playwright() as playwright:
    ...
    page.get_by_role("button", name="Send our details").click()
    assert page.locator("#form-status").inner_text() == "Please check the highlighted fields."
    fields = {"first_initial": "E.", "last_name": "Browser Example", "email": "browser@example.com",
    ...
    page.get_by_role("button", name="Send our details").click()
    page.wait_for_function("!document.querySelector('[type=submit]').disabled")
    assert page.locator('[name="email"]').input_value() == "browser@example.com"
    ...
    assert json.loads(target.read_text(encoding="utf-8"))["email"] == "browser@example.com"
    target.unlink()  # Only this test's known fictional submission.
    assert page.evaluate("document.activeElement.id") == "confirmation"
    page.get_by_role("button", name="Send details for another household").click()
    assert page.locator('[name="email"]').input_value() == ""
    page.goto("http://127.0.0.1:8080/welcome.html")
    page.wait_for_url("http://127.0.0.1:8080/")
